...
3. **Dağıtmadan önce maliyeti yaz, sonunda ölçüleni yaz.** Uzman çağırmak
   ölçülebilir para harcar: `ajans-os` üzerinde altı uzmanlık tek bir dalga
   60 saniyede **8,83 USD** tuttu. Beş dalgalık tam bir tarama bunun katıdır.
   Dört uzmandan fazlasını çağıracaksan önce "bu dalga <n> uzman çağıracak,
   ölçülen emsali dalga başına yaklaşık 9 USD" diye yaz ve **kullanıcının
   onayını bekle**.
